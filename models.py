...
# DBURI = 'mysql+mysqldb://%s:%s@%s:3306/%s?charset=utf8' % (config['dbuser'],config['dbpass'], config['dbhost'], config['dbname'])

# db = SQLAlchemy(DBURI)
import os

CURDIR = os.path.dirname(os.path.abspath(__file__))
dbfile = os.path.join(CURDIR, 'db.sqlite')
db = SQLAlchemy("sqlite:///%s" % dbfile, echo=False, pool_size=0)
